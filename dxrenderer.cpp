...
dx_renderer::dx_renderer(IDirect3DDevice9* device) {
	...
	D3DXCreateFont(m_device, 12, NULL, FW_HEAVY, 1, FALSE, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, (LPCWSTR)"Tahoma", &m_font);
}
...
void dx_renderer::draw_text(std::string text, int x, int y, unsigned long color, bool center, bool outline) {
	if (center) {
