...
void dx_overlay::initoverlay(std::wstring target_class, std::wstring target_window, bool not_topmost)
{
...
void dx_overlay::create_overlay(std::wstring target_class, std::wstring target_window) {
	...
	wc.lpszClassName = (LPCSTR)"dx_overlay";
	wc.hIconSm = NULL;
	...
	//Create our window
	m_overlay_wnd = CreateWindowExA(ex_styles, "dx_overlay", "", WS_POPUP | WS_VISIBLE,
	...
	//MARGINS m = { m_target_wnd_size.left, m_target_wnd_size.top, m_target_wnd_size.width(), m_target_wnd_size.height() }; //caused crashing for some reason
	MARGINS m;
	m.cxLeftWidth = m_target_wnd_size.left;
	m.cxRightWidth = m_target_wnd_size.top;
	m.cyTopHeight = m_target_wnd_size.width();
	m.cyBottomHeight = m_target_wnd_size.height();
